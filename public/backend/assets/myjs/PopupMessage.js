$(document).ready(function() {
    ...
    $('.customDataTable').DataTable({
        ...
        "ajax": {
            url: baseUrl + "/admin/data-table-popup-message",
            type: "POST",
            data: function(d) {
                d.table = table;
                d.column = columnName;
...
$(document).on('click', '.tqrpkskkgg', function() {
    ...
    $.ajax({
        type: "POST",
        url: baseUrl + "/admin/save-popup-message",
        data: $("#popupFormMessage").serialize() + "&_token=" + token,
        context: this,
        success: function(data) {
            ...
            if (json.status == "success") {
                ...
                setTimeout(function() {
                    window.location.replace(baseUrl + "/admin/popup-message")
                }, 300);
            } else {
