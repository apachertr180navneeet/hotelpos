$(document).ready(function() {
    ...
    $('.customDataTable').DataTable({
        ...
        "ajax": {
            url: baseUrl + "/admin/purchase-order/data-table-purchase-order",
            type: "POST",
            data: function(d) {
                d.table = table;
                d.column = columnName;
...
$(document).ready(function(e) {
    $('#purchaseForm').on('submit', (function(e) {
        ...
        if (error == false) {
            ...
            $.ajax({
                type: 'POST',
                url: baseUrl + "/admin/purchase-order/submit-purchase",
                data: formData,
                cache: false,
                contentType: false,
                processData: false,
                context: this,
                 success: function(data) {
                    ...
                    if (json.status == "success") {
                        ...
                        setTimeout(function() {
                            window.location.replace(baseUrl + "/admin/purchase-order");
...
$(document).on('keyup', '.pnaekqlgpu', function() {
    ...
    if (search == "") {
        ...
        $.ajax({
            type: 'POST',
            url: baseUrl + "/admin/purchase-order/get-products",
...
$(document).on('click', '.jpcljhzbiw', function() {
    ...
    $.ajax({
        type: 'POST',
        url: baseUrl + "/admin/purchase-order/add-to-purchase",
...
$(document).on('click', '.ulknpowlqk', function() {
    ...
    $.ajax({
        type: 'POST',
        url: baseUrl + "/admin/purchase-order/delete-product-from-purchase",
        data: "productId=" + productId + "&_token=" + token,
        success: function(data) {
            var json = $.parseJSON(data);
            if (json.status == "success") {
...
$(document).on('click', '.grunmikftv', function() {
    ...
    if (error == 1) {
        ...
        $.ajax({
            type: 'POST',
            url: baseUrl + "/admin/purchase-order/quick-add-product",
            data: formData,
            cache: false,
            contentType: false,
            processData: false,
...
$(document).on('keyup', '.rbvacamrpi', function() {
    ...
    $.ajax({
        type: 'POST',
        url: baseUrl + "/admin/purchase-order/update-purchase-quantity",
...
$(document).on('blur', '.uazwgwilhi', function() {
    ...
    $.ajax({
        type: 'POST',
        url: baseUrl + "/admin/purchase-order/update-cost-price",
...
$(document).on('change', '.umnoiilcdp,.xsxebbmlkw', function() {
    ...
    $.ajax({
        type: 'POST',
        url: baseUrl + "/admin/purchase-order/update-tax-type",
...
function purchaseTotal(){
    ...
    $.ajax({
        type: 'POST',
        url: baseUrl + "/admin/purchase-order/purchase-total",
        data: "&_token=" + token,
        success: function(data) {
            var json = $.parseJSON(data);
            if (json.status == "success") {
...
$(document).on('click', '.changePurchaseStatus', function() {
    ...
    $.ajax({
        type: 'POST',
        url: baseUrl + "/admin/purchase-order/change-status",
...
$(document).on('click', '.avhktsiydv', function() {
    var context = $(this);
    $.ajax({
        type: 'POST',
        url: baseUrl + "/admin/purchase-order/get-vendor-data",
        data: "vendorId="+$(this).attr("data-vendorId")+"&purchaseOrderId="+$(this).attr("data-id")+"&_token="+token,
        success: function(data) {
            var json = $.parseJSON(data);
            if (json.status == "success") {
                $(".moyvwgpbmr").val(json.data.vendorId);
                $(".gayleaicri").val(json.data.heading);
                $(".sefzcqqgfa").val(json.data.email);
                $(".zgluujefew").val($(context).attr("data-id"));
                $("#sendMailToVendorPO").modal("show");
            }
...
$(document).on('click', '.wdmxisfomn', function() {
    var context = $(this);
    var vendorId = $(".moyvwgpbmr").val();
    var heading = $(".gayleaicri").val();
    var email = $(".sefzcqqgfa").val();
    var adminEmail = $(".vskbzmasky").val();
    var purchaseOrderId = $(".zgluujefew").val();
    $.ajax({
        type: 'POST',
        url: baseUrl + "/admin/purchase-order/send-po-mail",
        data: "vendorId="+vendorId+"&heading="+heading+"&email="+email+"&adminEmail="+adminEmail+"&purchaseOrderId="+purchaseOrderId+"&_token="+token,
        success: function(data) {
            var json = $.parseJSON(data);
            if (json.status == "success") {
...
$(document).on('click', '.zdhntayosz', function() {
    $("#sendMailToVendorPO").modal("hide");
});
