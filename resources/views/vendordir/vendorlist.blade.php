...
@section('content')
        ...
        <div class="auto-container">
            <div class="title-outer" style="">
                ...
                <ul class="page-breadcrumb text-left">
                    <li><a href="{{ route('vendordirectory') }}">Category</a></li>
                    <li>Vendors</li>
                </ul>  --}}
                <h5 class="page-breadcrumb text-left" style="font-size: 20px;font-weight: bolder; margin-left: 6.5%;">
                    <a href="{{ route('vendordirectory') }}">Travel Nursing Industry Vendor Directory</a>
                    / 
                    <a href="{{ route('vendorSubList', ['vendor_category_slug' => $category->slug]) }}">
                        {{ $category->title }}
                    </a>
                    @if ($subcategory)
                        /
        ...
        <div class="row">
            <div class="col-md-3">
                <div class="form-group search-form-type">
                    ...
                    id="openListCompanyModel" style="width: 60%;margin-top: 3%;">
                    List Your Company
                    </a>
                </div>                                                            
            </div>
        ...
        @foreach ($vendors as $key => $vendor)
            <div class="card my-3 p-3">
                <div class="row">
                    <div class="col-md-2">
                        <div class="img-box">
                            @if(!empty($vendor->logo) && file_exists(public_path('uploads/vendoragency/' . $vendor->logo)))
                                <img src="{{ asset('public/uploads/vendoragency/' . $vendor->logo) }}" 
                    ...
                    <div class="col-md-2 text-end">
                        ...
                        @if ($vendor->products_count > 0)
                            <p class="company-info">
                                <a href="{{ route('vendorProducts', $vendor->id) }}"><i class="fa fa-gear icons-class" aria-hidden="true"></i> {{ $vendor->products_count }}
                                @if ($vendor->products_count == 1)
                                    product
                                @else
                                    products
                                @endif 
                        ...
                        @if ($vendor->blogs_count > 0)
                            <p class="company-info">
                                <a href="{{ route('vendorBlogs', $vendor->id) }}"><i class="fa fa-file icons-class"
                                        aria-hidden="true"></i> {{ $vendor->blogs_count }} 
                                    @if ($vendor->blogs_count == 1)
                                        Blog
                                    @else
                                        Blogs
                                    @endif
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#compnayForm').on('submit', function(event) {
                event.preventDefault();
            
                let formData = new FormData(this); // `this` is now the actual form element
            
                $.ajax({
                    url: "{{ route('vendordirectory.submit-company') }}", // Update with correct route name
                    method: "POST",
                    data: formData,
        ...
        $(document).ready(function() {
            ...
            $('#contactForm').on('submit', function(event) {
                event.preventDefault();
                $.ajax({
                    url: "{{ route('vendordirectory.mail.send') }}",
                    method: "POST",
                    data: $(this).serialize(),
                    success: function(response) {
