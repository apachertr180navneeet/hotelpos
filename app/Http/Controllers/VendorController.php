...
class VendorController extends Controller
{
    ...
    public function vendorcategorySearch(Request $request)
    {
        ...
        if ($filter === 'category') {
            // Search by category or subcategory title
            $categories = VendorCategory::leftJoin('vendor_agency_category', 'vendor_agency_category.vendor_categories_id', '=', 'vendor_categories.id')
                ->leftJoin('vendor_subcategories', 'vendor_subcategories.vendor_category_id', '=', 'vendor_categories.id')
                ->leftJoin('vendor_agencies', 'vendor_agencies.id', '=', 'vendor_agency_category.vendor_agencies_id')
                ...
                ->when(!empty($search), function ($query) use ($search) {
                    $query->where(function ($q) use ($search) {
                        $q->where('vendor_categories.title', 'LIKE', "%{$search}%")
                        ->orWhere('vendor_subcategories.title', 'LIKE', "%{$search}%");
                    });
                })
                ->orderBy('vendor_categories.title', 'asc')
            ...
            $vendors = VendorAgency::where('company_name', 'LIKE', "%{$search}%")
                ->withCount([
                    'products',
                    'blogs',
                    'releases',
            ...
            // Get first related category (optional logic, can be changed)
            $vendorCategoryLink = VendorAgencyCategory::whereIn('vendor_agencies_id', $vendorIds)->first();

            $category = $vendorCategoryLink 
                ? VendorCategory::find($vendorCategoryLink->vendor_categories_id)
                : null;

            $subcategory = ''; // Optional: can be enhanced to find actual subcategory if needed

            return view('vendordir.vendorlist', compact('vendors', 'subcategory', 'vendor_categories', 'vendor_sub_categories', 'category'));
        }

        // If filter is neither 'category' nor 'company', redirect or return error view
    ...
    {        
        ...
        return view('vendordir.vendorsubcategory', compact('categories', 'vendor_categories','category_slug','sub_cat_agency_count','vendor_sub_categories'));
    }
    ...
    public function vendorList($category_slug,$sub_category_slug)
    {
        ...
        $category = VendorCategory::where('id', $subcategory->vendor_category_id)->first();
